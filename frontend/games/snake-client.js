...
let _pendingBody = null; // one-step prediction while server processes our turn
...
function initSnakeClient(socket, myId) {
  ...
  socket.on("snake-state", (state) => {
    ...
    if (me && me.alive && me.body.length >= 2) {
      ...
      if (_lastDir === null) _lastDir = serverDir; // init on first state

      if (serverDir && serverDir !== _lastDir) {
        // Server hasn't processed our turn yet — show body one step ahead in our direction
        const d = DELTA[_lastDir];
        _pendingBody = [
          { x: me.body[0].x + d.x, y: me.body[0].y + d.y },
          ...me.body.slice(0, -1)
        ];
      } else {
        _pendingBody = null; // server caught up, no lookahead needed
      }
    } else {
      _pendingBody = null;
    }
...
function drawState(state) {
  ...
  [...dead, ...alive].forEach(snake => {
    ...
    const body   = (isMe && _pendingBody) ? _pendingBody : snake.body;
